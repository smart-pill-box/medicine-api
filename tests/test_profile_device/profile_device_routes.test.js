...
describe("POST /device/:deviceKey/profile_device", async ()=>{
    test("Returns 404 if device does not exist", async () => {
        await createDevice();
    ...
    test("Returns 404 if profile is not from right account", async ()=> {
        const { deviceKey } = await createDevice();
        const { accountKey } = await createAccount();
        const { profileKey } = await createProfile(accountKey);
    ...
    test("Returns 201 and create profileDevice on DB", async ()=>{
        const { deviceKey } = await createDevice();
        const { accountKey } = await createAccount();
        const { profileKey } = await createProfile(accountKey);
