...
describe("POST /device", async () => {
    test("Returns 400 if deviceKey is not provided", async () => {
        const response = await postDevice({});

        expect(response.status).toBe(400);
        expect(response.body.code).toBe("SCHEMA_ERR");
    });
    test("Returns 400 if additional properties provided", async () => {
        const response = await postDevice({
            deviceKey: uuidv4(),
            other: "other"
        });
    ...
    test("Creates device on DB with right key and returns 201", async () => {
...
describe("GET /device/:deviceKey", async ()=>{
    test("Return 404 if device don't exist", async ()=>{
        await createDevice();
    ...
    test("Return 200 and right device", async ()=>{
